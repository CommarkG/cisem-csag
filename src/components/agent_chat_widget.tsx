    ...
    <div className="fixed bottom-5 right-5 z-50 font-sans" dir="rtl">
      ...
      {!isOpen && (
        <button
          ...
          className="flex items-center gap-2.5 px-5 py-3.5 bg-slate-900 border border-red-500/30 hover:border-red-500 text-white rounded-full shadow-2xl transition-all transform hover:scale-105"
        >
        ...
        <div className="w-[360px] sm:w-[400px] h-[520px] bg-slate-900 border border-slate-800 text-slate-100 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="bg-slate-800 p-4 border-b border-slate-700 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-red-500/10 text-red-500 rounded-lg flex items-center justify-center text-lg">
                🤖
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs">
            {messages.map((msg) => (
              ...
              >
                {msg.role === 'assistant' && (
                  <div className="w-8 h-8 rounded-full bg-red-500/10 text-red-500 flex items-center justify-center shrink-0 border border-red-500/20 text-sm">
                    🤖
                  </div>
                )}
                ...
                {msg.role === 'user' && (
                  <div className="w-8 h-8 rounded-full bg-slate-750 text-slate-300 flex items-center justify-center shrink-0 text-sm border border-slate-700">
                    👤
                  </div>
                )}
              </div>
            ...
            {isLoading && (
              <div className="flex gap-2 text-slate-400 text-xs items-center justify-start">
                <span className="animate-spin text-red-500 text-sm">✨</span>
          ...
          <form onSubmit={handleSendMessage} className="p-3 bg-slate-800/50 border-t border-slate-800 flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2.5 text-xs text-slate-100 focus:outline-none focus:border-red-500 placeholder-slate-500 text-right"
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim()}
              className="p-2.5 bg-red-650 hover:bg-red-600 disabled:opacity-50 text-white rounded-xl transition-all"
            >
              <span>➡️</span>
            </button>
          </form>
