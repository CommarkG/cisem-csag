...
metadata:
  ...
  canonical_location: "C:\\Users\\finky\\Desktop\\AntiGravity\\Cisem CsAg\\2026-08-10__Gemini3.5__YarivHuman__CoreSpiralDecouplingImplementationPlan__V1.3.md"
  artifact_status: "RATIFIED"
  maturity: "RATIFIED"
  version: "1.3"
  governor_signature: "GOV-YARIV-20260810-CORE-SPIRAL-V1.0"
  role_type: "CORE_SPIRAL_PLAN"
  author: "Gemini 3.5 (Antigravity)"
  related_axioms: ["AX-10000", "AX-20000", "AX-60000", "PR-11100", "PR-11500", "PR-13990", "AX-SPIRAL-01", "AX-SPIRAL-02", "AX-SPIRAL-03", "AX-SPIRAL-04", "AX-SPIRAL-05", "PR-105000"]
...
This document establishes the **Version 1.3 CoreSpiral plan** to decouple the **CISEM Deep Platform Core** from the **Universal Solution Core** and deploy the multi-tenant user hierarchy. It integrates the permanent mechanical enforcements (Phases 15–17 in `cisem_gate.py`, `PlanIngestor.py` V0.3, and `AxiomsAndPrinciples` V1.30 upgrades) ratified during the consensus review. Following Governor instructions, CSAG is treated strictly as a temporary sandbox use-case to keep design focus on the universal platform decoupling.

1.2. **Methodology Rationale**:
Deploying user-roles join graphs, template registries, and row-level security in a single monolithic action introduces high regression risks. This plan strictly enforces the **CoreSpiral Methodology** (`AX-SPIRAL-01` to `AX-SPIRAL-05`):
...
- **Cross-Cycle Prerequisite Locking**: No cycle code may execute until the prior cycle's outputs are verified.
...
graph TD
    ...
    subgraph "CoreCycle 4: Gate Hardening & Promotion Governance (CC4)"
        ...
        P --> Q["Implement Phase 15-17 in platform_core/cisem_gate.py"]
    end
```
...
- 5.4.4. **Action Steps**:
  ...
  - Update `CisemSync.py` and registry hashes to reflect new gate location.
  ...
  - **Hardwire Gate Mechanical Enforcements (Phases 15-17)**:
    - **Phase 15 (CoreCycle Prerequisite Scanner)**: Reads active plan's header and prevents compile if predecessor dependencies are not verified.
    - **Phase 16 (DDL Integrity Scanner)**: Rejects DDL migration checks targeting JSONB fields for credentials, roles, feature flags, or missing FKs on tenant fields.
    - **Phase 17 (CoreCycle Exit Verification)**: Reads `scratch/proof_cc{N}.json` execution telemetry and locks cycle advance if exit codes are non-zero.
...
The Phase 2 PlanIngestor validator is updated to V0.3 to inspect CoreSpiral plans for:
...
- **2026-08-10 (Version 1.2)**: Integrated permanent mechanical enforcements (Phase 15 CoreCycle dependency scanner, Phase 16 DDL integrity checker, and Phase 17 executable exit proof checker) into the plan, along with PlanIngestor V0.3 specification.
- **2026-08-10 (Version 1.3)**: Plan ratified by Governor Yariv (GOV-YARIV-20260810-CORE-SPIRAL-V1.0). Switched system to execution mode. Appended database migrations 25-33 and set up directory splits under CoreCycle 1.
