...
**Date**: 2026-09-01  
**Version**: 1.2 (Ratified Specification with Physical Table Naming Distinction, UUID Keys, and Resolution Function)  
...
- Core platform terms cannot be mutated or deleted by tenants. Stale or retired terms are never deleted from database tables—they are superseded via explicit UUID deprecation pointers.

1.3. **Mandatory Table Naming Distinction Law**:
- **A table that exists in live PostgreSQL DDL is named strictly as it physically exists (`quotes`, `inquiries`, `catalog_items`, `vocabulary_terms`, `customer_accounts`).**
- **A table that does not yet exist carries its physical layer prefix (`cr_null_flavors`, `ext_mto_proofs`) because it will be created with one in Stage 1 DDL.**
- `public.cr_ext_registry` serves as the permanent classification authority mapping live table layers without breaking live database functions (`get_active_tenant_id()`) or foreign key constraints.
...
- Categorical columns reference `vocabulary_terms(id)` via UUID foreign keys or dedicated primary key tables (`cr_null_flavors(code)`, `status_library(code)`). Mutating protected core terms triggers PostgreSQL error `23514 (check_violation)` or custom trigger refusal.
...
- **The Corrected UUID Foreign Key Target**:
  - `vocabulary_terms.code` is NOT unique across kinds and tenants. Therefore, `superseded_by_code VARCHAR` cannot be a Foreign Key target.
  - `vocabulary_terms` carries two explicit supersession columns: `is_deprecated BOOLEAN DEFAULT false`, `superseded_by_id UUID REFERENCES vocabulary_terms(id)`.
  - Target `vocabulary_terms.id` is a guaranteed Primary Key (`UUID`), ensuring 100% DDL constraint validity.
- **Behavior**:
...
- No raw string literal representing a category or status may be stored in any database column without an explicit Foreign Key to `status_library`, `cr_null_flavors`, or `vocabulary_terms(id)`.
...
9.1. **NEXT TWO STEPS**:
1. **REVIEWER CLAUDE & GOVERNOR YARIV**: Reviewer Claude verifies Sub-Artifact A2 V1.2 (with live physical table names `quotes`, `inquiries`, `catalog_items`) and Governor Yariv ratifies.
2. **ANTIGRAVITY**: Upon verification of A2 V1.2, proceed to draft Sub-Artifact A3 (The Domain Architecture & Extension Specification).
