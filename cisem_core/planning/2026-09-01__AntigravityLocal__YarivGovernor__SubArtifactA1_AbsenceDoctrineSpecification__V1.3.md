...
**Date**: 2026-09-01  
**Version**: 1.3 (Ratified Specification with Physical Table Naming Distinction & Dedicated Null Flavors Table)  
...
- **Deterministic Columns (95% of fields)**: Single unambiguous null meaning documented in PostgreSQL DDL column comment (e.g. `COMMENT ON COLUMN quotes.valid_until IS 'Absence (NULL) indicates DER: Pending approval decision.';`).
...
- *Attack*: A developer executes `ALTER TABLE quotes ADD COLUMN internal_notes TEXT;` without adding a DDL comment or null reason column.
...
9.1. **NEXT TWO STEPS**:
1. **REVIEWER CLAUDE & GOVERNOR YARIV**: Reviewer Claude verifies Sub-Artifact A1 V1.3 (with live physical table names `quotes`, `inquiries`) and Governor Yariv ratifies.
2. **ANTIGRAVITY**: Upon verification of A1 V1.3, proceed to update Sub-Artifact A2 V1.2 to live physical table names.
