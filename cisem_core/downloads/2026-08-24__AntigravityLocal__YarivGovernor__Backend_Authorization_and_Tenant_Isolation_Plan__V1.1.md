...
metadata:
  ...
  canonical_location: "C:\\Users\\finky\\Desktop\\AntiGravity\\Cisem CsAg\\cisem_core\\planning\\2026-08-24__AntigravityLocal__YarivGovernor__Backend_Authorization_and_Tenant_Isolation_Plan__V1.1.md"
...
> [!IMPORTANT]
> **Refusal Behavior Standard & Specific Target Routes**: Silent fallback to `tenant_demo` or empty array responses (`[]`) on missing authentication will be strictly removed.
> The two specific target routes currently exhibiting silent fallback behavior are:
> 1. `GET /api/v1/tenant/whitelabel` in [`backend/src/backend/main.py` L551](file:///C:/Users/finky/Desktop/AntiGravity/Cisem%20CsAg/backend/src/backend/main.py#L551) (currently falls back to demo tenant config).
> 2. `GET /api/v1/crm/deals` in [`backend/src/backend/main.py` L1197](file:///C:/Users/finky/Desktop/AntiGravity/Cisem%20CsAg/backend/src/backend/main.py#L1197) (currently returns an empty array `[]` without HTTP 401/403 refusal).
> Both routes will be refactored to return explicit HTTP `401 Unauthorized` or HTTP `403 Forbidden` responses when authentication or authorization fails.
...
## Assumptions Carried Without Re-Checking

1. **Database RLS Invariants:** PostgreSQL Row Level Security policies defined in `backend/src/backend/migrations.sql` remain valid and active.
2. **Role Taxonomy:** `role_definitions` table in `backend/src/backend/migrations.sql` (L477–486) contains the 7 canonical roles (`platform_admin`, `account_owner`, `account_admin`, `team_manager`, `member`, `viewer`, `client`).
3. **Manifest Integrity:** `cisem_core/planning/ratified_plans_manifest.json` remains the sole authority for ratified plan IDs and Governor signatures.
