...
const CORE_DIR = path_1.default.dirname(__dirname);
...
app.post("/route", async (req, res) => {
    ...
    const checkpointsDir = path_1.default.join(CORE_DIR, "trials", "checkpoints");
    fs_1.default.mkdirSync(checkpointsDir, { recursive: true });
    const cpPath = path_1.default.join(checkpointsDir, `TRIAL-001__Checkpoint-${today}.json`);
    let cpData = [];
    if (fs_1.default.existsSync(cpPath)) {
        try {
            cpData = JSON.parse(fs_1.default.readFileSync(cpPath, "utf8"));
        }
        catch {
            cpData = [];
        }
    ...
    fs_1.default.writeFileSync(cpPath, JSON.stringify(cpData, null, 2), "utf8");
